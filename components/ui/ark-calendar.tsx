...
export default function ArkCalendar({ date, setDate }: ArkCalendarProps) {
    ...
    return (
        <DatePicker.Root
            ...
            selectionMode="range"
            className="p-3 bg-card text-card-foreground rounded-lg border shadow-sm inline-block"
        >
            <DatePicker.Context>
                {(api) => (
                    <>
                        ...
                        <DatePicker.View view="day">
                            <DatePicker.ViewControl className="flex justify-between items-center mb-4">
                                <DatePicker.PrevTrigger className="p-1 hover:bg-muted rounded text-muted-foreground cursor-pointer"><ChevronLeftIcon className="w-4 h-4" /></DatePicker.PrevTrigger>

                                <DatePicker.ViewTrigger className="text-sm font-bold text-foreground hover:bg-muted px-2 py-1 rounded cursor-pointer transition-colors">
                                    {/* SỬA: Custom Title cho Day View: "tháng 2 - 2025" */}
                                    tháng {api.visibleRange.start.month} - {api.visibleRange.start.year}
                                </DatePicker.ViewTrigger>

                                <DatePicker.NextTrigger className="p-1 hover:bg-muted rounded text-muted-foreground cursor-pointer"><ChevronRightIcon className="w-4 h-4" /></DatePicker.NextTrigger>
                                ...
                                <DatePicker.TableHead>
                                    <DatePicker.TableRow>
                                        {api.weekDays.map((weekDay, id) => (
                                            <DatePicker.TableHeader key={id} className="text-xs text-muted-foreground font-medium w-9 h-9 text-center">{weekDay.narrow}</DatePicker.TableHeader>
                                        ))}
                                    </DatePicker.TableRow>
                                </DatePicker.TableHead>
                                        ...
                                        <DatePicker.TableRow key={id}>
                                            {months.map((month, id) => (
                                                <DatePicker.TableCell key={id} value={month.value} className="p-1">
                                                    <DatePicker.TableCellTrigger className="w-full px-2 py-2 text-sm rounded hover:bg-primary/10 text-muted-foreground font-medium cursor-pointer data-[selected]:bg-primary data-[selected]:text-primary-foreground">
                                                        {month.label}
                                                    </DatePicker.TableCellTrigger>
                                                </DatePicker.TableCell>
                                            ))}
